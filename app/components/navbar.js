...
export default function Navbar() {
    ...
    return (
          <>
            ...
            <div className="navbar fixed flex justify-between flex-row top-0 z-10 h-16 w-screen px-10 p-5">
                  <div className="flex flex-row justify-start items-center  ">
                    <a 
                    className="btn btn-ghost bg-transparent text-xl text-white flex justify-center items-center hover:cursor-pointer"
                   ...
                   <div className="hidden lg:flex flex-row gap-x-6 items-center justify-center ">
                    <button 
                    className="my-auto hover:cursor-pointer text-white hover:underline" 
                    onClick={() => window.location = "/"}
                    >
                      Home
                    </button>
...
function Sidebar({open, setOpen}) {
    return (
        <>
            ...
            <div className={`fixed w-screen flex flex-col z-20 min-h-64 bg-[#0a1929] border-b border-[#334e68] duration-500 transition-transform ${ open ? "translate-y-0" : "-translate-y-full"} `}>
                <div className="h-16 w-full px-10 p-5 inline-flex items-center justify-between border-b border-gray-500">
                    <h1 className="text-xl font-bold text-white">RELLZ</h1>
                    <button className="text-xl text-white" onClick={() => {setOpen(false)}}><FaRegCircleXmark /></button>
                </div>
                <ul className="px-5 p-3 text-white flex flex-col gap-4 text-lg py-5">
